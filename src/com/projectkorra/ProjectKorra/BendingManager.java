	...
	public void handleDayNight() {
		for (World world: Bukkit.getServer().getWorlds()) {
			if (!times.containsKey(world)) {
				...
				AtomicBoolean isDay = times.get(world);
				if (isDay.get() && !Methods.isDay(world)) {
					...
					sendFirebenderMessage(world, isDay);
					if(isNight(world)) {
						sendWaterbenderMessage(world, isDay);//isNight also checks if the world is nether or end type
					}
				}
				...
				if (!isDay.get() && Methods.isDay(world)) {
					...
					isDay.set(true);
					sendFirebenderMessage(world, isDay);
                                        sendWaterbenderMessage(world, isDay);//isNight works the same as !isDay if it's day time
                                                                             //iit will always return false if it's night, no matter the world type
					}
				}
			}
		}
